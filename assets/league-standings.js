...
// Callout element for “entries remaining”
...
async function loadLeagueStandings() {
  ...
  try {
    ...
    // Determine max week present in picks (fallback to 18)
    let maxWeek = 0;
    allPicks.forEach((p) => {
    ...
    // Build lookup: entryId -> Map(week -> pick)
    ...
    // 3) Build stats per entry, computing elimination from picks
    const stats = entries.map((entry) => {
      const entryPicks = picksByEntry.get(entry.id) || new Map();

      // Compute whether entry has at least one LOSS
      let hasLoss = false;
      ...
      // Base isActive on picks; if the DB explicitly marks them inactive,
      // respect that as well (manual override).
      let isActive = !hasLoss;
    ...
    // 4) Update “entries remaining” callout
...
function renderLeagueTable(stats, weeks) {
  ...
  stats.forEach((row) => {
    ...
    // Optional class for eliminated rows (for CSS styling, e.g. dim text)
    if (!row.isActive) {
